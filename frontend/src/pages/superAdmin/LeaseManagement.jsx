...
export default function LeaseManagement({ user, onLogout }) {
  ...
  const fetchLeases = async () => {
    try {
      setLoading(true);
      const res = await api.get('/super-admin/leases');
      setLeases(res.data);
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    ...
    try {
      await api.patch(`/super-admin/leases/${id}/status`, payload);
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <div className="flex-1 pl-60">
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-20 shadow-sm">
          <div>
            <h1 className="text-lg font-bold text-slate-800">Lease Management</h1>
          </div>
        </header>
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {leases.map((lease) => (
                    <tr key={lease.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-4 font-mono text-xs text-slate-500">{lease.warehouse_id || '—'}</td>
                      <td className="p-4">
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase ${
                          lease.status === 'approved' || lease.status === 'active'
                            ? 'bg-green-50 text-green-600' 
                            : lease.status === 'rejected' || lease.status === 'terminated'
                      ...
                      <td className="p-4 pr-6 text-right space-x-2">
                        ...
                        {(lease.status === 'approved' || lease.status === 'active') && (
                          <button
                            onClick={() => handleUpdateStatus(lease.id, 'terminated')}
                            className="px-2.5 py-1 text-xs font-bold text-rose-500 border border-rose-500/20 hover:bg-rose-50 rounded-lg cursor-pointer"
                          >
                            Terminate
                          </button>
